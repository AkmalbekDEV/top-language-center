...
const DynamicPage = () => {
    ...
    const getData = async () => {
        try {
            const response = await axios.get(`https://fe100a235e57a25f.mokky.dev/courses/${params.id}`)
    ...
    return (
        <div className='max-w-[1250px] mx-auto mt-44'>
            <div className='flex items-center gap-3 max-sm:ml-8'>
                ...
                <p><BsArrowRight /></p>
                <a href="" className='hover:underline'>{data.name}</a>
            </div>
            <div className='mt-8 flex items-center gap-10 max-sm:gap-10 max-sm:flex-col max-sm:px-10 max-sm:text-center'>
                <img src={data.img} className='w-[40%] max-sm:w-full shadow shadow-blue-700 rounded-xl' alt="" />
                <div className='grid gap-10 p-5 rounded-2xl shadow-xl bg-white w-full py-12'>
                    <p className='text-2xl max-sm:text-lg font-medium flex items-center gap-3'><BsWatch size={30} /> Continuity: <span className='text-black'>{data.duration} months</span></p>
                    <p className='text-2xl max-sm:text-lg font-medium flex items-center gap-3'><ImPriceTag size={30} /> Price: <span className='text-black'>{data.price} sum</span></p>
                    <p className='text-2xl max-sm:text-lg font-medium flex items-center gap-3'><GiTeacher size={30} /> Teacher: <span className='text-black'>{data.teacher}</span></p>
                </div>
            </div>
            ...
            <div className='flex justify-center gap-10 max-sm:flex-col mt-10 max-sm:px-5'>
                ...
                <form id='ref' onSubmit={handleData} className='rounded-xl shadow-xl w-[400px] h-[300px] max-sm:w-full p-5 grid gap-5 bg-blue-700'>
                    ...
                    <input value={name} onChange={(e) => setName(e.target.value)} type="text" required name='name' placeholder="Your name..." className='w-full py-2 pl-5 text-lg border-2 rounded-xl outline-none' />
                    <input defaultValue={"+998"} value={number} onChange={(e) => setNumber(e.target.value)} type="tel" required name='number' placeholder="Your number..." className='w-full py-2 pl-5 text-lg border-2 rounded-xl outline-none' />
                    <div className='flex justify-center'>
                        <button className='w-[50%] hover:shadow-md hover:shadow-yellow-500 transition-all rounded-xl text-white text-lg font-medium bg-[#f7b602] py-2'>Sign Up</button>
                    </div>
                </form>
            </div>
        </div>
